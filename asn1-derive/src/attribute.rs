...
use syn::{self, spanned::Spanned, Data, Error, Meta, NestedMeta};
...
impl<T> Attribute<T> {
    ...
    pub fn set<Span: Spanned>(&mut self, span: Span, value: T) -> Result<(), Error> {
        if self.value.is_none() {
            ...
            Err(Error::new(
                span.span(),
                format!("asn1: duplicate attribute: {}", self.name),
            ))
        }
    }
}
...
impl Container {
    pub fn parse(item: &syn::DeriveInput) -> Result<Self, Error> {
        ...
        for meta_item in item
            .attrs
            .iter()
            .filter_map(|a| parse::attributes(a).emit())
            .flatten()
        {
            match meta_item {
...
impl Field {
    pub fn parse(item: &syn::Field) -> Result<Self, Error> {
        ...
        Ok(Self {
            name: name
                .value
                .or_else(|| item.ident.clone().map(|i| i.to_string())),
            implicit: implicit.value,
            explicit: explicit.value,
        })
    }
